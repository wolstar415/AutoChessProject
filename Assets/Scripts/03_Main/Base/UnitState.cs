...
namespace GameS
{
    public class UnitState : MonoBehaviourPunCallbacks
    {
        ...
        public bool NoHeal;
        [Header("특성계열 확인")] public bool Job3 = false;
        ...
        protected IEnumerator IHpSlider(float time = 0.4f)
        {
            float curTime = 0;
            float check = HpSliderCheck.value;
            while (curTime<time)
            {
                if (HpSliderCheck.value+Mathf.Epsilon <= HpSlider.value)
                {
                    break;
                }

                curTime += Time.deltaTime;
                HpSliderCheck.value = Mathf.Lerp(check, HpSlider.value, curTime/time);
        ...
        protected IEnumerator DotDamgeFunc(GameObject card, GameObject target)
        {
            ...
            while (cnt <= 10)
            {
                if (!PlayerInfo.Inst.IsBattle || PlayerInfo.Inst.BattleEnd || card_stat.IsDead || target_stat.IsDead ||
                    !target_stat.NoHeal)
                {
                    yield break;
                }
        ...
        protected void RPC_Item34Func(int targetnum, float d, int pidx)
        {
            ...
            DamageManager.inst.DamageFunc1(gameObject, ob, d, eDamageType.Spell_Magic);
